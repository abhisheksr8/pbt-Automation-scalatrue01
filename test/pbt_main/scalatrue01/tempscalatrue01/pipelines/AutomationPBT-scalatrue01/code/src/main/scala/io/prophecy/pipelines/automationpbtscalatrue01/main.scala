package io.prophecy.pipelines.automationpbtscalatrue01
...
import io.prophecy.pipelines.automationpbtscalatrue01.graph._
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/AutomationPBT-scalatrue01"
    )
    registerUDFs(spark)
    MetricsCollector.instrument(spark, "pipelines/AutomationPBT-scalatrue01") {
      apply(context)
    }
  }
